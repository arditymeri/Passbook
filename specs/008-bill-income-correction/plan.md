...
| IV. Currency Precision | ✅ Pass, with a documented exception | `BigDecimal` throughout. The negative-amount reversal is a deliberate, system-only exception (see Constraints above) — never exposed to a user as an input, always produced by the new correction/removal services. **Revised during PR review:** the two request schemas this feature introduces (`correctBillRequest`, `correctIncomeRequest`) originally carried `amount` as `format: double`. Since they are new here, the 007 "pre-existing gap" exemption never applied to them — both now use a pattern-validated decimal **string**, parsed straight to `BigDecimal` in the controller, with the frontend sending its existing string state unparsed. The pre-existing `format: double` on the `bill`/`income`/`account` **response** schemas (flagged in 007) is untouched and still out of scope — fixing it is a breaking change across every existing endpoint. |
